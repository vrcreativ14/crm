        ...
        <div className='insurer-info shadow-1'>
            <ul className='quote-short-detail'>
                ...
                <li className='action-column d-flex d-md-table-cell justify-content-center'>
                    {('coverage_type' in insurer && insurer.coverage_type=='basic') ? <button className='btn-nexus btn-golden me-3 me-md-0' onClick={() => setShowBasicPopup(true)}>{CurrencyFormat(insurer.total_premium,true,insurer.currency)}<br/>Yearly</button>
                    :
                    <ActionButton customClass="me-3 me-md-0" url={'summary/'+insurer.id} text={CurrencyFormat(insurer.total_premium,true,insurer.currency)+'<br/>Yearly'}/>
                    }
                    <button className="btn-nexus btn-grey" onClick={() => setShowDetails(!showDetails)}>Details</button>
