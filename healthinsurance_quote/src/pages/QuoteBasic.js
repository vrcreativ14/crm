...
const QuoteBasic = () => {
    ...
    const fetch = async () => {
        const res = await GetData('basic',navigate)
        if(res)setData(res)
    }
    ...
    return(
        <Layout currentTab={currentTab} name={name} stepContent={stepContent} comparison={startComparison}>
            ...
            <div className='text-center font-size-medium mt-5 fw-bold fst-italic'>Not what you are looking for? Please <a className='text-golden' href={'/health-insurance-form/get-quotes/start/123/'+data.deal.referrer+'/'+data.deal.deal_id}>click here</a> to go back and change your indicative budget.</div>
        </Layout>
    )
}
