...
from mortgage.models import Deal


class TaskBaseView(LoginRequiredMixin, CompanyAttributesMixin,
                   PermissionRequiredMixin, AjaxListViewMixin, TemplateView):
    template_name = "mortgage/tasks_list.html"
    permission_required = 'auth.list_mortgage_tasks'
    default_order_by = 'due_datetime'
    default_filter = 'todo'

    def get_queryset(self):
        ContentType = apps.get_model("contenttypes", "ContentType")
        content_type = ContentType.objects.get(app_label="mortgage", model="deal")
...
class TaskSingleView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    permission_required = 'auth.update_mortgage_tasks'
    model = Task
...
class AddEditTaskView(CoreAddEditTaskView):
    permission_required = 'auth.create_mortgage_tasks'
    model = Task
    attached_model = Deal
