...
class TaskBaseView(LoginRequiredMixin, CompanyAttributesMixin,
    ...
    template_name = "motorinsurance/task/tasks_list.djhtml"
    ...
    def serialize_object_list(self, records):
        ...
        return [{
            'pk': record.pk,
            'title': record.title,
            'attached_to': f'{record.attached_to.customer.name} ({record.attached_to.get_car_title()})',
            'attached_to_url': reverse('motorinsurance:deal-edit', kwargs={'pk': record.object_id}),
            'deal_tags': record.attached_to.get_tags() or None,
            'deal_premium': self.get_deal_premium(record.attached_to),
            'deal_sum_insured': '{:,}'.format(record.attached_to.vehicle_insured_value),
            ...
            'update_task_field_url': reverse('motorinsurance:task-update-field', kwargs={'pk': record.pk}),
...
class AddEditTaskView(AddEditTaskView):
