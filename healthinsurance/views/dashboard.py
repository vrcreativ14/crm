...
class BaseChartDataView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get_chart_data(self, querytype=None):
        ...
        if querytype == "healthdeals":
            ...
            self.renewal_deals = Deal.objects.filter(stage='lost', deal_type = DEAL_TYPE_RENEWAL).order_by('created_on')
            if  self.params.get('user'):
                self.deals = self.deals.filter(user = self.params.get('user'))
            ...
            # import pdb
            # pdb.set_trace()
            self.deals = Order.objects.filter(deal__stage = STAGE_WON).exclude(deal__deal_type = DEAL_TYPE_RENEWAL).order_by('created_on')
            self.renewal_deals = Order.objects.filter(deal__stage = STAGE_WON, deal__deal_type = DEAL_TYPE_RENEWAL).order_by('created_on')
            if self.params.get('user'):
...
class DealsFilter():
    def __init__(self, queryset, params):
        ...
        if self.params.get('filtertype') == "date":
            ...
            renewals = self.deals.filter(deal_type = DEAL_TYPE_RENEWAL)
            new_deals = self.deals.exclude(deal_type = DEAL_TYPE_RENEWAL)            
            ...
            for sd, ed in month_ranges:
                ...
                chart_data.append((
                    month_label, new_deals.filter(created_on__range=(sd, ed)).count(), renewals.filter(created_on__range=(sd, ed)).count()
                ))
...
class HealthLostDealsView(BaseChartDataView):
    def get(self, request):
        ...
        deals_filter = DealsFilter(self.deals, self.params)
        renewal_deals_filter = DealsFilter(self.renewal_deals, self.params)
        return JsonResponse(deals_filter.get_result(), safe=False)
...
class HealthSalesConversionRateView(BaseChartDataView):
    def get(self, request):
        ...
        for sd, ed in month_ranges:
            ...
            deals = deal_qs.filter(created_on__range=(sd, ed)).exclude(deal_type = DEAL_TYPE_RENEWAL)
            renewal_deals = deal_qs.filter(created_on__range=(sd, ed), deal_type = DEAL_TYPE_RENEWAL)
            
            if self.params.get('insurer'): # If insurer filtering out won deal from "Order" model.
                deals_won_qs = deals_won.filter(created_on__range=(sd, ed)).exclude(deal__deal_type = DEAL_TYPE_RENEWAL)
                deals_won_renewal_qs = deals_won.filter(created_on__range=(sd, ed), deal__deal_type = DEAL_TYPE_RENEWAL)
            else:
                deals_won_qs = deals.filter(Q(stage = 'won'))
                deals_won_renewal_qs = renewal_deals.filter(Q(stage = 'won'))
            ...
            renewal_deals_count = renewal_deals.count()
            ...
            if renewal_deals_count > 0:
                conversion_rate_renewal = deals_won_renewal_qs.count() / float(renewal_deals_count)
            else:
                conversion_rate_renewal = 0.0

            chart_data.append((
                month_label, '{:0.2f}'.format(conversion_rate * 100.0), '{:0.2f}'.format(conversion_rate_renewal * 100.0)
            ))
...
class HealthOrdersTotalPremiumView(BaseChartDataView):
    ...
    def get(self, request):
        ...
        renewals = base_qs.filter(deal__deal_type = DEAL_TYPE_RENEWAL)
        renewals_premium = 0
        if  self.params.get('insurer') and self.params.get('filtertype') == 'date':
...
class HealthDealByInsurer(BaseChartDataView):
     def get(self, request):
        ...
        for insurer in Insurer.objects.all():
            ...
            chart_data.append((insurer.name, filtered_orders.count(), self.renewal_deals.count()))
        # for sd, ed in month_ranges:
        #     month_label = sd.strftime('%b, %y')
        #     if self.params.get('insurer'):
        #         total_premium = (
...
class GetRecords(BaseChartDataView):
    def get(self, request):
