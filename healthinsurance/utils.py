...
def filter_deals(*args, **kwargs):
    data = []
    search_term = kwargs.get('filters').get('search')
    stage_filter = kwargs.get('filters').get('stage')
    status_filter = kwargs.get('filters').get('status')
    user = kwargs.get('filters').get('user')
    from_date = kwargs.get('filters').get('from_date')
    to_date = kwargs.get('filters').get('to_date')
    ...
    if request_user and request_user.userprofile.has_producer_role():
        deals = deals.filter(Q(user = request_user) | Q(referrer = request_user))
